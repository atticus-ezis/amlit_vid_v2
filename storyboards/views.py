...
styles = {
    "pixar": "in a pixar semi realistic style"
}
...
def storyboard_view(request, blueprint_pk):
    ...
    if request.method=="POST":
        ...
        if action == "generate_character_sheets":
            ...
            completed_sheets=Image.objects.filter(
            ...
            if number_of_needed_sheets != completed_sheets.count():
    ...
    existing_character_sheets=Image.objects.filter(
        characters__in=blueprint.characters.all()
...
def generate_character_design_sheet_images(characters: list[QuerySet], size: str, style: str) -> list[Image]:
    batch_number=5
    character_count=characters.count()
    image_count=1
    first_char = characters.first()
    if not first_char:
        raise ValueError("No character provided to 'def generate_character_design_sheet_images'")
